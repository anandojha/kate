...
- **Core codec** - Kabsch alignment, TICA collective variables, a from-scratch RealNVP
  ...
  coder, and a run-aware MSM, packaged as the `kate` CLI
  (`compress` / `decompress` / `analyze` / `bound` / `benchmark`).
- **The kinetic bound** (`kate.pathbound`) - the path-distribution KL factorized into an
  ensemble term + a transition term, with Pinsker bounds on kinetic observables. The
  organizing principle of the project.
- **T1** path bound wired into the runner + `kate bound` (pure-numpy contrast scorer).
- **T2** production kinetics via deeptime (`kate analyze`: lag scan, Bayesian error bars).
- **T3** baseline-comparison harness (`kate benchmark`) with SZ3/ZFP/MDZip subprocess
  wrappers (located via `KATE_SZ3_BIN` / `KATE_ZFP_BIN` / `KATE_MDZIP_DIR`) + local
...
- **T10** the kinetic path-bound made **differentiable as a training loss**
  (`kate.bound_loss`): a soft-MSM transition term `h(P‖Q)` with autograd, so
...
- **Kinetic-resolution accounting** (`kinetics_deeptime.kinetic_resolution`,
  `kate analyze --resolution`): per dynamical process, the Bayesian timescale + 95% CI,
...
- **The contrast, measured on real NTL9, resolution-aware** (`docs/ntl9_contrast_resolved.png`):
  to preserve the resolved-band kinetics to <1%, KATE needs ~12 bits/frame vs SZ3 ~840 /
...
- **T9 gate measured on real NTL9 (both halves)** - rate
  ...
  because KATE compresses slow CVs that stay strongly autocorrelated (ρ≈0.98) even at
...
- **Code review (physics + competitive) and its fixes** (`docs/REVIEW.md`): audited the
  ...
  - **Certified kinetics now use deeptime's reversible MLE** (`estimate_reversible_T`,
    ...
    portable `kate bound` stays torch/deeptime-free on the symmetrized estimator.
  - **`kate analyze --mfpt N`**: PCCA+ metastable coarse-graining + mean first-passage
    times - the k_on/k_off rate observables the bound covers, now actually computed.
  - **`kate compress --features contacts`**: invariant inter-atomic-distance TICA
  ...
  - **Path bound honors support**: `kinetic_bound_valid` / Pinsker = inf when the true
    divergence is +∞ (a missed transition); `kate bound` warns.
  - Honest competitive positioning in README (KATE is not a pure-compression winner).
...
- `deeptime` is an optional `[kinetics]` extra; `compress` / `decompress` / `bound` run
  on the core dependencies alone, and `import kate` pulls in neither torch nor deeptime.
