# KATE - Full Code Review (physics validity + competitive comparison)

A four-axis review of KATE against the reference packages it builds on or competes with
...
- **On pure compression (rate, all-atom RMSD, speed, maturity) KATE is the weakest of the
...
**B4 - [HIGH-value gap] KATE promises k_on/k_off and MFPTs but computes neither.**
...
1. [x] **DONE** - **Routed certified kinetics through deeptime's reversible MLE** (B1,
   ...
   `kate bound` remains the portable (C+Cᵀ)/2 scorer, since deeptime pulls in torch.
...
5. [x] **DONE** - **MFPT** reporting (B4, `4276d07`): `kate analyze --mfpt N` computes mean
...
7. [x] **DONE** - **Internal-coordinate (contacts) featurization** (`1ed16e6`):
   `kate compress --features contacts` (invariant inter-atomic distances; removes spurious
