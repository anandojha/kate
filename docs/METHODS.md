...
A molecular-dynamics (MD) trajectory is most often compressed to save storage, but
what scientists actually want preserved is not the raw coordinates — it is the
...
- The **ensemble term** bounds errors in *static* observables (populations, free
  energies). This is what ensemble-preserving and coordinate-bounded compressors
  control.
...
Components are reused prior art where credibility matters (deeptime's reversible-MLE
MSM, BayesianMSM, VAMPnets, streaming covariance) and reimplemented from the
published algorithm where ownership and reproducibility matter (the flow, the
Witten–Neal–Cleary range coder, the path bound, IGFS, the artifact format, the CLI).
No third-party compressor source enters the repo; SZ3/ZFP/MDZip are external
...
Invariant across the track: the flow stays invertible, the bound stays intact, and
there is **no lossy CNN autoencoder** (that is MDZip's design and it breaks the
thesis). Among the ML pieces only T8/T10 are positioned as novel; VAMPnets, spline
...
timescale is sampling-limited and is excluded, by name, rather than reported as if it
...
the **same** centers, then scored on the resolved band.
...
**0.8 % timescale error at 10 bits/frame vs static's 1.5 % at 26** — same kinetic
...
raw-coordinate MSE — what SZ3/ZFP minimize — spends **0 bits** on the slow coordinate
...
wins at higher budget (3.4 % vs 13.8 % timescale error). Cause: the real MSM
...
irrelevant — *surrogate and estimator disagree*. Aligning them (soft states matched to
the discretization) is the open problem. Reported, not hidden.
...
- **Sampling, not compression, limits NTL9's slow kinetics.** The folding timescale
  (~tens of µs) has < 1 independent event in 25 µs; no featurization or compressor
...
- **The contrast compares different objectives** (§6.1); the honest axis is
...
- **Compression ratio is not the headline** — kinetic fidelity at a given rate is.
- **Validation here is NTL9 only.** A well-sampled ligand-binding system (e.g.
  trypsin-benzamidine) would test binding kinetics and the slow band the present data
  cannot resolve.
