...
def main():
    ...
    Q_good = _stochastic(P_ref + 1e-3 * rng.random(P_ref.shape))   # ~ KATE retained MSM
    ...
    for name, Q in [("Q_slow  (ensemble-only; rates x%.1f)" % c, Q_slow),
                    ("Q_good  (retained MSM ~ KATE)", Q_good)]:
    ...
    print("~1/c = %.1fx. The transition term exposes exactly that gap; KATE retains" % (1.0 / c))
