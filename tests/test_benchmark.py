"""T3 contrast harness. The deliverable is the CONTRAST: EPC's transition term ~0
...
def test_benchmark_writes_contrast_png(tmp_path):
    ...
    run_benchmark([coords], methods=["epc", "shuffle"], lag=10, nstates=30,
...
def test_external_baseline_unavailable_is_skipped():
    ...
    res = run_benchmark([coords], methods=["epc", "sz3"], lag=10, nstates=20,
                        verbose=False)
    by = {r["method"]: r for r in res}
    assert by["epc"].get("available") is True
