"""T3 contrast harness. The deliverable is the CONTRAST: KATE's transition term ~0
...
from kate.benchmark import run_benchmark
from _synth import metastable_coords


def test_contrast_kate_zero_shuffle_large():
    coords = metastable_coords(n_steps=4000, n_atoms=8, seed=0)
    res = run_benchmark([coords], methods=["kate", "shuffle", "quantize"],
                        lag=10, nstates=40, out=None, verbose=False)
    by = {r["method"]: r for r in res if r.get("available")}
    # KATE retains the MSM -> transition term identically ~0
    assert by["kate"]["transition_kl"] < 1e-9
    # an ensemble-only resample destroys kinetics: large transition term ...
    assert by["shuffle"]["transition_kl"] > 1e-2
    assert by["shuffle"]["transition_kl"] > 1e6 * by["kate"]["transition_kl"] + 1e-3
...
def test_benchmark_writes_contrast_png(tmp_path):
    ...
    run_benchmark([coords], methods=["kate", "shuffle"], lag=10, nstates=30,
...
def test_external_baseline_unavailable_is_skipped():
    ...
    res = run_benchmark([coords], methods=["kate", "sz3"], lag=10, nstates=20,
                        verbose=False)
    by = {r["method"]: r for r in res}
    assert by["kate"].get("available") is True
