...
from epc.cli import main, _load_reference_counts
...
def test_bound_with_matrix_ref_and_shape_mismatch(tmp_path, capsys):
    q = str(tmp_path / "q.epc"); save_artifact(toy_artifact(a=0.05, seed=1), q)
...
def test_decompress_full_atom_without_residual(tmp_path, capsys):
    ...
    from epc.codec import gaussian_cumfreq, encode_iid
    ...
    p = str(tmp_path / "nf.epc"); save_artifact(art, p)
...
def test_analyze_explicit_lags(tmp_path, capsys):
    ...
    p = str(tmp_path / "k.epc"); save_artifact(kinetics_artifact(n_steps=3000, nstates=20, lag=10), p)
...
def test_analyze_requires_deeptime(tmp_path, monkeypatch):
    p = str(tmp_path / "k.epc"); save_artifact(toy_artifact(), p)
    import epc.kinetics_deeptime as kd
...
def test_runner_helpers_free_energy_and_kl():
    from epc.runner import free_energy_1d, kl_1d
...
def test_streaming_vampnet_is_rejected(tmp_path):
    ...
    with pytest.raises(SystemExit):
        run_epc(pdb, dcd, streaming=True, cv="vampnet", verbose=False)


def test_print_report_all_optional_branches(capsys):
    pytest.importorskip("torch")
    from epc.runner import compress_trajectory, print_report
